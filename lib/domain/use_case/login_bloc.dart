...
import 'package:tikka_demo_app/domain/entity/login_state.dart';
...
class LoginBloc extends UseCaseBloc<LoginState, LoginIntent>  {
  
  LoginBloc() : super(LoginState()) {
    on<PerformLogin>((event, emit) async {
    LoginService service = get<LoginService>();

      emit(state.copy(loadingStatus: DataLoadingStatus.loading));
      ...
      if (response is Success) {
        emit(state.copy(loadingStatus: DataLoadingStatus.success, error: null));
      } else {
        emit(state.copy(loadingStatus: DataLoadingStatus.failure));
      }
    }); 
  }
}